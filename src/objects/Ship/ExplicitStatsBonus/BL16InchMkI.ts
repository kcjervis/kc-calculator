...
const createBonus: StatsBonusCreator = ship => {
  ...
  if (shipClass.is("NelsonClass")) {
    bonus.add({ multiplier: count, firepower: 2, armor: 1 })
  } else if (shipClass.is("QueenElizabethClass")) {
    bonus.add({ multiplier: count, firepower: 2, armor: 1, evasion: -2 })
  } else if (shipClass.is("KongouClass") && shipNameIsKai2(ship.name)) {
    bonus.add({ multiplier: count, firepower: 1, armor: 1, evasion: -3 })
  }
