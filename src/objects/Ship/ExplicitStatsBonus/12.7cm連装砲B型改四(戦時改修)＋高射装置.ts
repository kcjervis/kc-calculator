...
const createBonus: StatsBonusCreator = ship => {
  ...
  if (ship.hasGear(gear => gear.is("SurfaceRadar"))) {
    if (shipClass.any("AyanamiClass", "AkatsukiClass", "HatsuharuClass")) {
      bonus.add({ firepower: 1, torpedo: 2, evasion: 2 })
    } else if (shipClass.is("ShiratsuyuClass")) {
      bonus.add({ firepower: 1, torpedo: 3, evasion: 2 })
    }
  }
  // 対空電探
  ...
  if (ship.hasGear(285)) {
    if (shipClass.any("AyanamiClass", "AkatsukiClass", "HatsuharuClass")) {
      bonus.add({ firepower: 1, torpedo: 3 })
    }
  }
  // 61cm四連装(酸素)魚雷後期型
  if (ship.hasGear(286)) {
